...
const populateUsers = async () => {
    try {
        let foundUsers = await User.find({age: {$gte: 30}}).
                            populate({
                                path: 'blogPosts', 
                                select: ['title', 'content'],//Hiện title, content
                                //match: {content: /lâu đài/i},
                                options: { limit: 5 }
                            }). //Chỉ hiện một số trường
                            //populate('tên trường khác') =>có thể populate các trường "tham chiếu khác"
                            exec()
...
const populateBlogPosts = async () => {
    try {
        let foundBlogPosts = await BlogPost.find({}).
                                populate('author', ['name', 'email']). //Chỉ hiện một số trường
                                exec()
